...
int main(int argc, char *argv[]) {
    ...
    if(rank == 0){
        printf("Enter M N K: \n");
        fflush(stdout);
        int n_read = scanf("%d %d %d", &M, &N, &K);
        if(n_read != 3){
            fprintf(stderr, "Error reading M, N, K\n");
    ...
    double *A = NULL;
    double *B = (double*)malloc(N*K * sizeof(double)); // Reused buffer for all ranks
    double *C_pt = NULL, *C_col = NULL, *C_seq = NULL;
...
/* =====================================================
   ...
   ===================================================== */
    ...
    if(rank == 0){
        // Send chunks of A and full B to all other ranks
        for(int p=1; p<size; p++){
            MPI_Send(A + p*rows*N, rows*N, MPI_DOUBLE, p, 0, MPI_COMM_WORLD);
            MPI_Send(B, N*K, MPI_DOUBLE, p, 0, MPI_COMM_WORLD);
        }
        // Copy own chunk
        for(int i=0; i<rows*N; i++) localA[i] = A[i];
        // B is already filled on rank 0
    } else {
    ...
    if(rank == 0){
        // Copy own results
        for(int i=0; i<rows*K; i++) C_pt[i] = localC[i];
...
/* =====================================================
   VERIFICATION (optional, not timed)
   ===================================================== */
    if(rank == 0){
